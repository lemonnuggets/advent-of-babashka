...
(defn strip-leading-zero [day] (str/replace day #"^0" ""))

(defn ensure-leading-zero [day] (if (= 1 (count day)) (str "0" day) day))

(defn new-day
  {:org.babashka/cli {:coerce {:day :string, :year :string}}}
  [{:keys [year day], :or {year "22"}}]
  (let [new-year   (str "aoc" year)
        day-number day
        new-day    (str "day" (ensure-leading-zero day-number))
    ...
    (let [new-day-resource (fs/file "resources" new-year (str new-day ".txt"))]
      (if-not (fs/exists? new-day-resource)
        (if-let [session (System/getenv "AOC_SESSION")]
          (let [input-url (format "https://adventofcode.com/20%s/day/%s/input"
                                  year
                                  (strip-leading-zero day-number))]
                ...
                (curl/get
                 ...
                 {:headers
                  {"Cookie" (str "session=" session),
                   "User-Agent"
                   "github.com/borkdude/advent-of-babashka-template by michielborkent@gmail.com"}}))]
              (spit new-day-resource body)))
          (do
            (println
             "Tip: set the AOC_SESSION environment variable to the Advent of Code session cookie to retrieve your input automatically!")
            (println
             "For now, a copy of the input of day 1 is copied as a stub.")
